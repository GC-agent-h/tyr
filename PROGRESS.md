...
- [x] **Phase-05→06 payload handoff: locate + structurally validate the real replication carrier in all 10 files** — CHARACTERIZED/DONE (t7). Decoder in `tools/carrier_decode.py` classifies + validates every `reassembled_payload`; ≥99% structural pass (B 99.07% exact-13B-with-cb, C 100% terminal-00). Per 2026-07-13 decision, ≥99% structural validation is accepted as sufficient for this sub-step; see plan doc "Acceptance criterion for carrier decode".
  - [ ] **U1 (OPEN, blocks downstream property decode): semantic decode of Family A blob + object-id resolution.** 
    ...
    - **Binary disassembly started (commit ad3c315):** `tools/binary_harness.py` (pefile + capstone) locates all 44 embedded Iris `*.cpp` PDB path strings, localizing replication modules in the symbol-stripped binary (ReplicationReader@0x96bd070, ReplicationOperationsInternal@0x96bca40, ReplicationWriter@0x96c2260, ObjectReplicationBridge@0x96b6440, NetRefHandleManager@0x96b3e60, etc.). **Carrier REFRAMED from data:** the Family-A/E body is a HIERARCHICAL OBJECT BUNDLE, not flat class state — `E_0100`/N=2 bodies = object-REFERENCE resolution block (`SerializeIntPacked(75)` + repeating `(u16 refIdx,28 01 a0,u16,81 12 00)` records); `A_large` N=31 (`ch=13`) = root actor + 30 subobjects (all ODD static keys) with a 1936B state blob; `ch=53/25` = the per-member property STATE serializer. So the u16 key→class mapping is NOT in the state blob — it is set at actor SPAWN (separate spawn bunch carrying the UClass path). U1 therefore requires reversing TYR's spawn-bunch class writer + the property-state serializer widths. **Disassembly IN PROGRESS** (module map done; per-member serializer extraction = next target).
